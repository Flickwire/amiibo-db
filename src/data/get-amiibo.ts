...
export async function getAmiiboById(id: string, context: QueryFunctionContext): Promise<AmiiboResponse> {
  return axios.get('',{params: {id},signal: context.signal}).then((res) => res.data).then((data) => AmiiboListSchema.parse(data))
}
